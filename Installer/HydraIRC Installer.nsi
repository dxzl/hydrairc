...
OutFile ..\Distribution\${PRODUCT}-v${VERSION}-TestVS2015.exe
...
; will be stored in the wrong place AND it may not be able to use WM_COPYDATA to talk
; with my YahCoLoRiZe Plugin for Hydra if YahCoLoRiZe runs as user and HydraIRC is
; running as admin.
;!include "MUI.nsh"
!include "MUI2.nsh"
!include "UAC.nsh"
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\hydrairc.exe" ;checkbox to run the prog (will now run at User level on Win 7, 8 10)
...
Section "${PRODUCT} Main Program Files (Required)" SecCore
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT}" "Contact" "#hydrairc@Freenode"
...
Function myLeaveFinishFunction
  ...
  !insertmacro INSTALLOPTIONS_READ $0 "ioSpecial.ini" "Field 4" "STATE"
  StrCmp $0 "0" +1 +2
  ...
  !insertmacro INSTALLOPTIONS_READ $0 "ioSpecial.ini" "Field 5" "STATE"
  StrCmp $0 "0" +1 +2
