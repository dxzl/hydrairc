...
#define UWM_MDICHILDHIDDEN_MSG             _T("UWM_MDICHILDHIDDEN_MSG-5DAD28E2-C961-11d5-8BDA-00500477589F") // Hydra
...
// Hydra - Start
#if (_MSC_VER >= 1300)
__declspec(selectany) UINT UWM_MDICHILDHIDDEN = ::RegisterWindowMessage(UWM_MDICHILDHIDDEN_MSG);
#else
// Initialize via RegisterTabbedMDIMessages
__declspec(selectany) UINT UWM_MDICHILDHIDDEN = 0;
#endif
// Hydra - End
...
struct RegisterTabbedMDIMessages
{
    ...
    UWM_MDICHILDHIDDEN =             ::RegisterWindowMessage(UWM_MDICHILDHIDDEN_MSG); // Hydra
	}
};
