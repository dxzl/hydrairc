...
public:
	...
	~CAutoHideBar()
	{
		/* For some reason this didn't correctly delete and destroy the CPinnedWindow objects
		   (especially the m_txt member variable). So let's do it the old fashioned way..
		void (*pDelete)(void *)=&(operator delete);
		std::for_each(m_bunch.begin(), m_bunch.end(), pDelete);*/
		for (CBunch::iterator i=m_bunch.begin(); i != m_bunch.end(); ++i) delete (*i);
	}
