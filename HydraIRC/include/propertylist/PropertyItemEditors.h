...
public:
   ...
   LRESULT OnKillFocus(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
   {
      if( ::GetFocus() != m_wndButton ) ::SendMessage(GetParent(), WM_USER_PROP_UPDATEPROPERTY, 0, (LPARAM) m_hWnd);
      bHandled = FALSE;
      return 0;
   }
   LRESULT OnKeyDown(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
...
{
public:
   DECLARE_WND_SUPERCLASS(_T("WTL_InplacePropertyCombo"), CEdit::GetWndClassName())

   CContainedWindowT<CListBox> m_wndList;
   HWND m_hWndCombo;  // Listbox supplyed by Property class
   int m_cyList;      // Used to resize the listbox when first shown
